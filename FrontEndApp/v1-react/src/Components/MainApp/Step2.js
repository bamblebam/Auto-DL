...
function Step2() {
	...
	const handleDragEnd = async ({ destination, source }) => {
		...
		if (destination.droppableId === "delete" && source.droppableId === "target") {
		...
		if (destination.droppableId === "target" && source.droppableId === "target") {
			...
			for (let [key0, value0] of Object.entries(dic)) {
				...
				for (var key1 in value0) {
					if (!(key1 === "name" || key1 === "id")) {
						if (key1 === "type") {
							final_dict[`activation_function-${i}-${dic[key0].name}-selected`] = true
						} else {
						...
						for (key2 in dic[key0][key1]) {
							if (key2 === "value") {
								if (dic[key0][key1].Datatype === "number") {
									...
									final_dict[`Layer-${i}-${dic[key0].name}-${key1}`] = parseFloat(
										dic[key0][key1][key2]
									)
								} else if (dic[key0][key1].Datatype === "Tuple") {
				...
				for (var key1 in value0) {
					if (!(key1 === "name" || key1 === "id")) {
						if (
							...
							if ("value" in dic[key0][key1] && dic[key0][key1]["value"] !== "") {
								continue
							} else {
								flag = false
								break
					...
					if (!(key1 === "name" || key1 === "id")) {
						if (key1 === "value") {
							if (selected_loss[key0][key1].Datatype === "number") {
								...
								final_dict[`${selected_loss.type}-${selected_loss.name}-${key0}`] = [
									parseInt(temp[0]),
									parseInt(temp[1]),
								]
							} else {
				...
				if (temp_pre[key][key1]) {
					for (var key2 of Object.keys(temp_pre[key][key1])) {
						if (key2 !== "name" && key2 !== "input_type")
							temp_dic[`${key}-${key1}-${key2}`] = temp_pre[key][key1][key2]["Default"]
					}
				} else {
			...
			<Dialog onClose={handleCloseModal} open={openModal}>
				...
				<DialogContent dividers>
					<div>
						...
						<ul>
							<li>
								Click the "Download Code" button to download the generated code to any
